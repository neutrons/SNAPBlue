...
def copyDifcal(refCal,cal,propagate=False): # refCal and Cal are CalibrationStatus dictionaries generated by checkCalibrationStatus
    ...
    newIE = IndexEntry(version=calDict["version"]+1,
        runNumber=refCalDict["runNumber"],
    ...
    print("this is the new index entry to be created: \n",newIE)
    ...
    if propagate:
        ...
        print(f"now up to version: {newIE.version}")
        ...
        print("updating calibration Record")
        newCR = frankenRecord(seedRun=seedRun,
            ...
            isLite=cal["isLite"],printRecord=True)
        
        print("test version")
        print(f"CR version: {newCR.version}")
        print(f"version type is: {type(newCR.version)}")
        print(f"IE version: {newIE.version}")
        print(f"version type is: {type(newIE.version)}")
        print("about to save calibration record/update index entry")
...
def reVersionDifcal(seedDir,destDir,seedRun,destRun):
    ...
    print("reversioning calibration files:")
...
def saveCalibrationRecord(calibrationRecord,indexEntry,destinationRun):
    ...
    localDataService=LocalDataService()
    print(f"Setting target index with run: {destinationRun} and useLite: {indexEntry.useLiteMode}")
    # targetIndexer = localDataService.calibrationIndexer(destinationRun,
    #                                                     indexEntry.useLiteMode
    #                                                     )
    # print(targetIndexer)
    print(f"""
          calling writeCalibrationRecord with

          calibrationRecord.runNumber = {calibrationRecord.runNumber}
          calibrationRecord.useLiteMode = {calibrationRecord.useLiteMode}
          
          """)
    localDataService.writeCalibrationRecord(calibrationRecord,indexEntry)
    # targetIndexer.writeCalibrationRecord(calibrationRecord,indexEntry)
...
def frankenRecord(seedRun,
    ...
    #need to rewrite workspaces

    franken.workspaces = {
        "diffCalOutput" :[f"dsp_column_{str(seedRun).zfill(5)}_v{str(destVersion).zfill(4)}"],
        "diffCalDiagnostic" :[f"diagnostic_column_{str(seedRun).zfill(5)}_v{str(destVersion).zfill(4)}"],
        "diffCalTable" :[f"diffract_consts_{str(seedRun).zfill(5)}_v{str(destVersion).zfill(4)}"],
        "diffCalMask":[f"diffract_consts_mask_{str(seedRun).zfill(5)}_v{str(destVersion).zfill(4)}"]
    }

    #TODO: do franken.workspaces need to be updated?
